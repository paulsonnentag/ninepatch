...
export function startsWith(names: readonly string[], prefix: readonly string[]): boolean {
  return prefix.length <= names.length && prefix.every((name, i) => names[i] === name)
}
...
function splitString(path: string): string[] {
  ...
  names.push(current)
  if (names.some((name) => name === "")) throw new Error(`empty name in path ${JSON.stringify(path)}`)
  return names
}
