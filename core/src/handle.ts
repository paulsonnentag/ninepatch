...
export function isHandle(x: unknown): x is Handle<unknown> {
  return typeof x === "object" && x !== null && (x as Record<PropertyKey, unknown>)[brand] === true
}
...
export function fromDoc<T>(handle: DocHandleLike<T>, options: { readOnly?: boolean } = {}): Handle<T> {
  const write = () => {
  ...
  return {
    ...
    set(next: T) {
      ...
      handle.change((doc) => {
        ...
        for (const key of Object.keys(target)) if (!(key in (next as object))) delete target[key]
...
export function field<T>(source: Handle<unknown>, path: string[]): Handle<T> {
  const read = (value: unknown): T =>
    path.reduce<unknown>((at, key) => (at as Record<string, unknown> | undefined)?.[key], value) as T
  return {
    [brand]: true,
    get value() {
...
export function derive<A, B>(source: Handle<A>, fn: (value: A) => B): Handle<B> {
  return {
    [brand]: true,
    get value() {
