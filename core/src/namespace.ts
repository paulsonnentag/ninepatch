...
export class NamespaceImpl {
  ...
  on(event: "change" | "destroy" | "open" | "close", fn: (...args: never[]) => unknown): () => void {
    if (event === "open") {
  ...
  get value(): unknown {
    ...
    if (result.kind !== "found" || !result.handle) throw new NotFound(this.relTarget(result.at))
...
async function resolveWithFills(
  ...
  for (let attempt = 0; attempt < 32; attempt++) {
    ...
    if (JSON.stringify(again.at) === key) throw new NotFound(requester.relTarget(again.at))
  }
  throw new NotFound(requester.relTarget(abs))
}

async function fireOpen(requester: NamespaceImpl, absTarget: string[]): Promise<void> {
  ...
  for (let ns: NamespaceImpl | undefined = requester; ns; ns = ns.parent) {
    ...
    for (const listener of [...ns.listeners.open]) {
      ...
      if (!out || typeof out.then !== "function") throw new TypeError("open handler must return a promise")
      calls.push(out)
    }
  }
...
class FromView {
  ...
  private rel(path: Path): string[] {
    ...
    if (!startsWith(abs, this.requester.pos)) throw new Error("fill outside the requester's subtree")
...
class Watch {
  ...
  private async run(generation: number): Promise<void> {
    ...
    try {
      const { result, fired: requests } = await resolveWithFills(this.ns, this.ns.pos)
      if (generation !== this.generation || this.stopped) return
