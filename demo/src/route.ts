...
export async function setupRoute(frame: Namespace, seed: Seed): Promise<void> {
  ...
  const location = (await frame.open<Route>("location")) as Namespace & Handle<Route>
  addEventListener("hashchange", () => location.set(parse(window.location.hash)))
  location.on("change", () => history.replaceState(null, "", toHash(location.value.docUrl)))
  frame.mount("selectedDoc", derive(location, (route) => route.docUrl)) // a derived URL is a live link
}
