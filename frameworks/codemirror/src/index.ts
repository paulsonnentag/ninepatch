...
export function bindText(view: EditorView, source: Handle<unknown>, path: string[]): () => void {
  let stopped = false
...
function read(value: unknown, path: string[]): unknown {
  return path.reduce<unknown>((at, key) => (at as Record<string, unknown> | undefined)?.[key], value)
}
